...
- Pack renamed **`comfyui-flow-production-tracking` → `flow-pt`** in `pyproject.toml`, `CLAUDE.md`,
  `DESIGN.md`, `README.md`, `COLD_START.md`, `__init__.py`, and the `custom_nodes` symlink.

Verified after a restart: `/workflow_templates` returns `{"flow-pt": [seven demos]}` and
`python_module` is `custom_nodes.flow-pt`.
...
The Templates browser builds its collection label from the `custom_nodes` directory name verbatim
(`title: e` in the frontend bundle), overridable only by a frontend i18n key a pack cannot ship.
The node's footer badge is `python_module` split on `.` — the same string. Registry names allow no
spaces, so `Flow PT` was never reachable, and the Registry spec advises against putting "ComfyUI" in
the name at all. The searched slots are `DisplayName` and `keywords`, which keep the long form.

Free to change because nothing is published: version 0.1.0, `PublisherId` empty.

**Not yet done:** the GitHub repo rename and the local checkout directory rename. Both are Kevin's
to run — the second breaks every open shell.
...
| GitHub repo rename to `flow-pt`, and the local checkout rename | Kevin |
...
- The templates collection label and the node footer badge are both the `custom_nodes` directory
  name. Neither reads `DisplayName`.
