...
3. **Does the profile match their show?** `profile.local.json`, keyed per project, at the path the
   ...
   site's own defaults, which suit a show that hangs Versions off Shots. Run `/inspect-site
   <project>` when the show names its Versions to a convention, hangs them off something else, or
...
4. **Provenance fields.** Settings, then SG, SG Site Setup says how many of the nine exist on this
   ...
   comfyui_sg.fields` instead. Without the fields a publish records everything in the Version's
   description, so this is worth doing and is not a blocker.
