// Captures Preview Image run from the selection toolbox, then SG Publish run the same way, ending
// on the rows the Version wrote.
...
// A picker draws the value it finds when it is built, so both are set before the node is added.
...
// The preview arrives at once, so wait for the image rather than for the queue.
