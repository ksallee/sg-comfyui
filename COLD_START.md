...
a Version with full provenance in Flow Production Tracking — and it took six wrong turns to get there. The wrong
...
5. Run `python -m comfyui_sg.fields` once to create the nine Version fields.
...
`/inspect-site`. `src/comfyui_sg/site.py` compounds it — its `profile()` docstring says the file is
...
the web extension (`web/sg_dom_widgets.js`, `web/sg_entity_picker.js`, `web/sg_panel.js`)
...
|---|---|---|---|
| `SGPublishVersion` | 11 | **14** | `link_pick`(4), `sg_panel`(12), `refresh from site`(13) |
| `SGLoadVersion` | 10 | **15** | `project_pick`(1), `link_pick`(3), `statuses_chips`(6), `sg_panel`(13), `refresh from site`(14) |

Everything after the first injected widget lands one or more slots off. On `SGLoadVersion` the
...
Actually true: the sibling is **`../sg-groundtruth`**, and `src/comfyui_sg/_deps.py` says
...
### D2. Every documented `python -m comfyui_sg.*` command is missing two required env prefixes

`/track-workflow` step 4b writes:

```
python -m comfyui_sg.seed <file> --project P --link "sh010 (Shot)" --output plate --note "..."
```

That fails. `comfyui_sg` lives under `src/`, so it is not importable, and `_deps.py` needs the
sibling path unless the checkout happens to sit beside `sg-groundtruth`. What actually works:

```
SG_GROUNDTRUTH_PATH=/path/to/sg-groundtruth PYTHONPATH=src python -m comfyui_sg.seed ...
```

Same omission applies to `python -m comfyui_sg.fields`, which `DESIGN.md` names.
...
`publish_version.next_code` → `naming.render`, is the Flow Production Tracking field-path form that `DESIGN.md`
documents correctly:
...
where the documented template would have given `cold_wancam_plate_v001`. `SGPublishVersion`'s
...
|---|---|---|---|
| F1 | `python -m comfyui_sg.seed` | **3** | verbatim → `ModuleNotFoundError` → `+PYTHONPATH=src` → `ImportError: set SG_GROUNDTRUTH_PATH` → `+SG_GROUNDTRUTH_PATH` |
...
| F5 | Running any CLI in a plain venv | **2** | `ModuleNotFoundError: numpy` — `comfyui_sg/__init__.py` imports the nodes, which import `numpy`, so **every setup-path CLI drags in the ComfyUI runtime deps**. Only `instrument.py` run as a file escapes this, and only because `/track-workflow` explicitly says "Run it as a file, not `-m`". |
...
- **Place `.env.local` in my worktree.** `.claude/settings.json` denies `Read(./.env.local)`, and
  ...
  `ComfyUI/custom_nodes/sg-comfyui` is a symlink to the main checkout.
...
- `example.png` seeded as **Version 31744** via `comfyui_sg.seed`, first try.
...
the provenance that lands in Flow Production Tracking is complete down to the lineage link — but **the single biggest
